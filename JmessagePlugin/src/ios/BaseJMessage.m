...
@implementation BaseJMessage

// 单例模式：只被GCD初始化一次
+(BaseJMessage*) getInstance{
    static BaseJMessage *baseJMessage = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        baseJMessage = [[self alloc] init];
    });
    return baseJMessage;
}
...
- (void)initBase:(UIApplication *)application
   launchOptions:(NSDictionary *)launchOptions
    ...
    [self registerJPushStatusNotification:observer];
...
- (void)jpushApplicationDidEnterBackground:(UIApplication *)application {
  DDLogDebug(@"Action - jpushApplicationDidEnterBackground");
