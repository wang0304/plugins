...
+(JCHATDetailsInfoViewController*) getInstance{
    static JCHATDetailsInfoViewController *instance = nil;
...
// 用户显示名称
//[((JMSGUser *)self.conversation.target) displayName];
...
-(void)addFriendsToGrouop:(NSString*)friendName{
    ...
    NSString *groupName = @"";
    if(friendName){
        NSString *owerName = ((JMSGUser *)self.conversation.target).username;
        groupName = [NSString stringWithFormat:@"%@,%@", owerName, friendName];
        memberArray = @[owerName, friendName];
    }

    [JMSGGroup createGroupWithName:groupName desc:@"" memberArray:memberArray completionHandler:^(id resultObject, NSError *error) {
